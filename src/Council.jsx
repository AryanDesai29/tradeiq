    ...
    return (
      <div key={m.id} className="c-seat" onClick={() => setDossier(m.id)} title={observer ? `${m.name} — observing this session` : `${m.name} — ${m.title}`}
        ...
        <div style={{ position: "relative", animation: speaking ? "cTalk 0.65s ease-in-out infinite" : observer ? "none" : `cBob ${3.2 + (i % 4) * 0.5}s ease-in-out ${i * 0.37}s infinite`, transformOrigin: "50% 100%" }}>
          ...
          <div style={{ marginTop: 2.5, fontSize: 12, fontWeight: 700, letterSpacing: "0.13em", textTransform: "uppercase", color: observer ? C.muted : m.color + "bb" }}>{observer ? "observing" : m.title.replace("The ", "")}</div>
          ...
          {v && <div style={{ marginTop: 3, display: "inline-block", fontSize: 12, fontWeight: 800, letterSpacing: "0.08em", padding: "2.5px 9px", borderRadius: 9, background: VOTE_COLOR[v.vote] + "26", color: VOTE_COLOR[v.vote], border: `1.5px solid ${VOTE_COLOR[v.vote]}66`, animation: "cVote 0.45s ease", fontFamily: C.display, textShadow: "0 1px 4px #000" }}>{v.vote.toUpperCase()}</div>}
        </div>
      </div>
    );
  };
        ...
        <div style={{ ...zone, animation: "cBubble 0.5s ease" }}>
          <div style={{ background: `linear-gradient(165deg,${C.s2}fa,${C.s1}fa)`, border: `1px solid ${VOTE_COLOR[vd.recommendation]}55`, borderRadius: 16, padding: 20, boxShadow: `0 24px 70px #000c, 0 0 50px ${VOTE_COLOR[vd.recommendation]}18`, maxHeight: "46vh", overflowY: "auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.22em", color: C.muted }}>THE CHAIRMAN'S VERDICT</div>
            ...
            <div style={{ fontSize: 12, marginBottom: 5, lineHeight: 1.6 }}>
            ...
            <div style={{ fontSize: 12, color: C.muted, marginBottom: 6 }}>{tally.consensus} · council score {tally.avg >= 0 ? "+" : ""}{tally.avg}<span style={{ fontSize: 12, color: C.dim }}> (avg vote, −2 to +2)</span></div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
              <div><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.16em", color: "#69f0ae", marginBottom: 4 }}>🐂 BULL CASE</div><div style={{ fontSize: 12.5, lineHeight: 1.6, color: C.text }}>{vd.bull_case}</div></div>
              <div><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.16em", color: "#ff5252", marginBottom: 4 }}>🐻 BEAR CASE</div><div style={{ fontSize: 12.5, lineHeight: 1.6, color: C.text }}>{vd.bear_case}</div></div>
            </div>
            {vd.key_risks.length > 0 && <div style={{ marginBottom: 10 }}><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.16em", color: C.gold, marginBottom: 4 }}>KEY RISKS</div>{vd.key_risks.map((x, i) => <div key={i} style={{ fontSize: 12, color: C.text, lineHeight: 1.6 }}>⚠ {x}</div>)}</div>}
            {vd.required_research.length > 0 && <div style={{ marginBottom: 10 }}><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.16em", color: "#ffd54f", marginBottom: 4 }}>🕵️ REQUIRED RESEARCH</div>{vd.required_research.map((x, i) => <div key={i} style={{ fontSize: 12, color: C.text, lineHeight: 1.6 }}>☐ {x}</div>)}</div>}
            <div style={{ background: C.accent + "10", border: `1px solid ${C.accent}30`, borderRadius: 8, padding: "10px 13px", marginBottom: 12 }}>
              <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.16em", color: C.accent }}>NEXT ACTION → </span>
              <span style={{ fontSize: 12.5, color: C.text }}>{vd.next_action}</span>
            </div>
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <button className="tiq-btn" onClick={() => { setPhase("idle"); setSession(null); setTopic(null); }} style={{ background: C.accent, border: "none", borderRadius: 8, color: C.bg, fontFamily: C.display, fontWeight: 800, fontSize: 12, letterSpacing: "0.08em", textTransform: "uppercase", padding: "10px 16px" }}>⚖ New Session</button>
              {source !== "live" && <button className="tiq-btn" onClick={() => convene(topic, { force: true, mode: session.mode || "quick" })} style={{ background: "none", border: `1px solid ${C.gold}50`, borderRadius: 8, color: C.gold, fontFamily: C.display, fontWeight: 800, fontSize: 12, letterSpacing: "0.08em", textTransform: "uppercase", padding: "9px 14px" }}>↻ Convene Fresh</button>}
              {source === "live" && session.mode === "quick" && <button className="tiq-btn" onClick={() => convene(topic, { force: true, mode: "full" })} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, color: C.muted, fontFamily: C.display, fontWeight: 800, fontSize: 12, letterSpacing: "0.08em", textTransform: "uppercase", padding: "9px 14px" }}>🏛️ Escalate to Full Council</button>}
              <span style={{ fontSize: 12, color: C.muted }}>Click any member to question their vote</span>
            </div>
          </div>
        </div>
      );
        ...
        <div key={turnIdx} style={{ ...zone, animation: "cBubble 0.35s ease" }}>
          <div style={{ background: `linear-gradient(170deg,${C.s2}f7,${C.s1}f7)`, border: `1.5px solid ${m.color}55`, borderRadius: 16, padding: "15px 19px", boxShadow: `0 18px 50px #000a, 0 0 36px ${m.color}16` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 12, color: C.muted, letterSpacing: "0.14em", textTransform: "uppercase" }}>{m.title}</span>
              {kd && <span style={{ marginLeft: "auto", fontSize: 12, fontWeight: 800, letterSpacing: "0.12em", color: m.color + "bb", border: `1px solid ${m.color}40`, borderRadius: 4, padding: "2px 8px" }}>{kd}</span>}
            </div>
    ...
    <div className="c-root" style={{ display: "flex", height: "100%", minHeight: 480, background: `radial-gradient(120% 90% at 50% 0%, #0c1426 0%, ${C.bg} 55%, #04060a 100%)`, position: "relative", overflow: "hidden" }}>
      ...
      <div className="c-chamber" style={{ flex: 1, position: "relative", minWidth: 0 }}>
        ...
        <div style={{ position: "absolute", top: 12, left: 0, right: 0, textAlign: "center", zIndex: 9, pointerEvents: "none" }}>
          ...
          {topic && phase !== "idle" && <div style={{ fontSize: 12, color: C.muted, marginTop: 3, maxWidth: 640, margin: "3px auto 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{TOPIC_TYPES.find((x) => x.id === topic.type)?.emoji} {topic.title}{session?.mode === "quick" && <span style={{ color: C.accent, marginLeft: 6, fontSize: 12, fontWeight: 700 }}>⚡ quick panel · {QUICK_IDS.length} of {COUNCIL.length} voices</span>}{source !== "live" && <span style={{ color: C.gold, marginLeft: 6 }}>↺ cached · 0 tokens{cachedAt && <span style={{ fontSize: 12, color: C.muted }}> · {relTime(cachedAt)}</span>}</span>}</div>}
        ...
        {(phase === "debate" || phase === "powers" || phase === "voting") && (
          <button className="tiq-btn" onClick={skip} style={{ position: "absolute", top: 14, right: 14, zIndex: 10, background: C.s2, border: `1px solid ${C.border}`, borderRadius: 7, color: C.muted, fontFamily: C.display, fontWeight: 700, fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", padding: "7px 13px" }}>Skip ≫</button>
        )}

        {/* 🏛 Council Treasury — themed usage ledger, health relative to own pattern */}
        <button className="tiq-btn" onClick={() => setShowTreasury((p) => !p)} style={{ position: "absolute", top: 14, left: 14, zIndex: 10, background: C.s2 + "e8", border: `1.5px solid ${healthColor}45`, borderRadius: 7, color: healthColor, fontFamily: C.display, fontWeight: 800, fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", padding: "7px 13px", boxShadow: "0 4px 10px #0008" }}>🏛 Treasury <span style={{ display: "inline-block", width: 7, height: 7, borderRadius: "50%", background: healthColor, marginLeft: 3 }} /></button>
        {showTreasury && (
          <div style={{ position: "absolute", top: 52, left: 14, zIndex: 13, width: 296, background: `linear-gradient(185deg,${C.s2}fa,${C.s1}fa)`, border: `2px solid ${C.gold}30`, borderRadius: 12, padding: 15, boxShadow: "0 18px 50px #000b", animation: "cBubble 0.25s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.1em", padding: "2.5px 9px", borderRadius: 9, background: healthColor + "1c", color: healthColor, border: `1px solid ${healthColor}45` }}>{treasury.h.label.toUpperCase()}</span>
            </div>
            {[["⚡ Quick sessions", treasury.s.quick], ["🏛️ Full sessions", treasury.s.full], ["💬 Member queries", treasury.s.asks],
              ...
              <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "4px 0", borderBottom: `1px dashed ${C.border}55` }}>
                <span style={{ color: C.muted }}>{l}</span><span style={{ color: C.text, fontFamily: C.mono, fontWeight: 700 }}>{v}</span>
              </div>
            ))}
            <div style={{ fontSize: 12, color: C.muted, marginTop: 8, lineHeight: 1.55 }}>Last 30 days · real Groq usage when reported · health is measured against your own pattern, not a hardcoded limit{treasury.h.ratio > 0 ? ` (today ≈ ${treasury.h.ratio}× your usual day)` : ""}.</div>
          </div>
        )}
        ...
        <div style={{ position: "absolute", left: "50%", top: "40%", transform: "translate(-50%,-50%)", width: "58%", height: "36%", borderRadius: "50%", background: "radial-gradient(ellipse at 50% 30%, #7a5530 0%, #5d3e20 36%, #432c14 68%, #2c1c0c 100%)", border: "5px solid #140b04", boxShadow: `0 30px 70px #000c, inset 0 12px 26px #ffffff1a, inset 0 -18px 34px #00000099, 0 0 70px ${C.accent}0a` }}>
          ...
          {phase === "idle" && !busy && (
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 12, color: C.muted, marginTop: 4, maxWidth: 300 }}>Bring an opportunity, trade, position, or thesis before the council below.</div>
              {cMode === "quick" && <div style={{ fontSize: 12, color: C.accent + "bb", marginTop: 6 }}>⚡ Quick panel: Sterling · Veris · Sigma · Popper</div>}
            </div>
          )}
          {busy && (
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              ...
              <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>reviewing your portfolio, track record & theses</div>
            </div>
          )}
        </div>
        ...
        {activePower && (
          <div style={{ position: "absolute", inset: 0, zIndex: 11, display: "flex", alignItems: "center", justifyContent: "center", background: "#04060acc", backdropFilter: "blur(2px)" }}>
            <div style={{ width: "min(500px,88%)", textAlign: "center", background: `linear-gradient(170deg,${C.s2},${C.s1})`, border: `2px solid ${activePower.color}66`, borderRadius: 18, padding: "30px 28px", boxShadow: `0 0 80px ${activePower.color}30`, animation: "cPower 0.5s ease" }}>
              ...
              <div style={{ fontSize: 12, color: C.muted, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 12 }}>{activePower.sub}</div>
          ...
          <div style={{ position: "absolute", left: "50%", bottom: 14, transform: "translateX(-50%)", width: "min(620px,94%)", zIndex: 9 }}>
            ...
            {suggestions.length > 0 && (
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 9, justifyContent: "center" }}>
                {suggestions.map((s, i) => <button key={i} className="tiq-btn" onClick={() => convene(s.t)} style={{ background: C.s2 + "ee", border: `1px solid ${C.accent}30`, borderRadius: 20, color: C.accent, fontFamily: C.mono, fontSize: 12, padding: "13px 14px" }}>{s.e} {s.l}</button>)}
                {lastSaved && <button className="tiq-btn" onClick={() => replay(lastSaved)} style={{ background: C.s2 + "ee", border: `1px solid ${C.border}`, borderRadius: 20, color: C.muted, fontFamily: C.mono, fontSize: 12, padding: "13px 14px" }}>↺ Replay last session</button>}
              </div>
            )}
            <div style={{ background: C.s1 + "f2", border: `1px solid ${C.border}`, borderRadius: 13, padding: 14, boxShadow: "0 14px 44px #000a" }}>
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                {COUNCIL_MODES.map((md) => <button key={md.id} className="tiq-btn" onClick={() => setCMode(md.id)} style={{ flex: 1, textAlign: "left", background: cMode === md.id ? (md.id === "quick" ? C.accent : C.gold) + "16" : "transparent", border: `1.5px solid ${cMode === md.id ? (md.id === "quick" ? C.accent : C.gold) + "60" : C.border}`, borderRadius: 9, padding: "9px 13px" }}>
                  <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase", color: cMode === md.id ? (md.id === "quick" ? C.accent : C.gold) : C.muted }}>{md.emoji} {md.label}{md.id === "quick" && <span style={{ fontSize: 12, marginLeft: 5, opacity: 0.8 }}>DEFAULT</span>}</div>
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>{md.hint}</div>
                </button>)}
              </div>
              <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
                {TOPIC_TYPES.map((t) => <button key={t.id} className="tiq-btn" onClick={() => setTType(t.id)} title={t.hint} style={{ background: tType === t.id ? C.accent + "1f" : "transparent", border: `1px solid ${tType === t.id ? C.accent + "60" : C.border}`, borderRadius: 7, color: tType === t.id ? C.accent : C.muted, fontFamily: C.display, fontWeight: 700, fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase", padding: "13px 12px" }}>{t.emoji} {t.label}</button>)}
              </div>
      ...
      <div className="c-rail" style={{ width: 362, flexShrink: 0, borderLeft: `1px solid ${C.border}`, background: C.s1 + "c8", display: "flex", flexDirection: "column", backdropFilter: "blur(4px)" }}>
        <div style={{ padding: "13px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          {session && <span style={{ fontSize: 12, color: C.muted }}>{playedTurns.length}/{session.transcript.length}</span>}
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px" }}>
          {!session && <div style={{ color: C.muted, fontSize: 12.5, lineHeight: 1.7, padding: "16px 4px", textAlign: "center" }}>The live transcript, ballots, and verdict will be recorded here once the council convenes.<div style={{ marginTop: 16, fontSize: 12, lineHeight: 2.1, textAlign: "left" }}>{COUNCIL.map((m) => <div key={m.id} style={{ cursor: "pointer" }} onClick={() => setDossier(m.id)}><span style={{ color: m.color, fontWeight: 700 }}>{m.emoji} {m.name}</span> <span style={{ color: C.dim }}>· {m.title}</span></div>)}</div></div>}
          {playedTurns.map((t, i) => (
            <div key={i} className="msg-in" style={{ marginBottom: 11, paddingLeft: 9, borderLeft: `2px solid ${memberById(t.member).color}50` }}>
              <div style={{ fontSize: 12 }}><MChip id={t.member} />{KIND_TAG[kindOf(t, i, session.transcript)] && <span style={{ color: C.muted, marginLeft: 6, fontSize: 12, letterSpacing: "0.1em" }}>{KIND_TAG[kindOf(t, i, session.transcript)]}</span>}</div>
              <div style={{ fontSize: 12.5, color: C.text, lineHeight: 1.6, marginTop: 3 }}>{t.text}</div>
            </div>
          ))}
          {session?.evidence_hold?.raised && phase !== "debate" && <div className="msg-in" style={{ margin: "11px 0", padding: "9px 12px", background: "#ffd54f12", border: "1px solid #ffd54f3a", borderRadius: 8, fontSize: 12, color: "#ffd54f", lineHeight: 1.55 }}>🕵️ EVIDENCE MISSING — {session.evidence_hold.demand}</div>}
          {session?.red_flags?.raised && phase !== "debate" && <div className="msg-in" style={{ margin: "11px 0", padding: "9px 12px", background: "#f48fb112", border: "1px solid #f48fb13a", borderRadius: 8, fontSize: 12, color: "#f48fb1", lineHeight: 1.55 }}>⚖️ RED FLAG REVIEW — {(session.red_flags.flags || []).join(" · ")}</div>}
          {shownVotes.length > 0 && <div style={{ margin: "14px 0 8px", fontSize: 12, fontWeight: 800, letterSpacing: "0.18em", color: C.muted }}>— BALLOTS —</div>}
          {shownVotes.map((v) => (
            <div key={v.member} className="msg-in" style={{ marginBottom: 9, paddingLeft: 9, borderLeft: `2px solid ${VOTE_COLOR[v.vote]}50` }}>
              <div style={{ fontSize: 12 }}><MChip id={v.member} /> <span style={{ fontWeight: 800, color: VOTE_COLOR[v.vote] }}>{v.vote} · {v.confidence}%</span></div>
              <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.5, marginTop: 2 }}>{v.reason}</div>
            </div>
          ))}
      ...
      {dossier && (() => { const m = memberById(dossier); const v = session?.votes.find((x) => x.member === m.id); const th = threads[m.id] || []; return (
        <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "min(400px,94%)", zIndex: 20, background: `linear-gradient(190deg,${C.s2}fb,${C.s1}fb)`, borderLeft: `2px solid ${m.color}50`, boxShadow: "-22px 0 60px #000b", display: "flex", flexDirection: "column", animation: "cBubble 0.3s ease" }}>
          <div style={{ padding: "18px 18px 14px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: C.muted }}>{m.title}</div>
              </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "14px 18px" }}>
            ...
            <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.65, marginBottom: 10 }}>{m.focus}</div>
            {m.power && <div style={{ fontSize: 12, color: m.color, background: m.color + "10", border: `1px solid ${m.color}30`, borderRadius: 8, padding: "8px 11px", marginBottom: 11, lineHeight: 1.55 }}>⚡ {m.power}</div>}
            {memories[m.id] && <div style={{ background: C.s2, border: `1px dashed ${m.color}38`, borderRadius: 8, padding: "9px 12px", marginBottom: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.16em", color: m.color, marginBottom: 4 }}>{m.name.toUpperCase()}'S READ ON YOU</div>
              <div style={{ fontSize: 12, color: C.text, lineHeight: 1.65 }}>{memories[m.id]}</div>
            </div>}
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
              {m.questions.map((q) => <button key={q} className="tiq-btn" disabled={askBusy} onClick={() => ask(m.id, q)} style={{ textAlign: "left", background: C.s3, border: `1px solid ${C.border}`, borderRadius: 8, color: C.accent, fontFamily: C.mono, fontSize: 12, padding: "13px 12px" }}>❝ {q} ❞</button>)}
            </div>
            {th.map((x, i) => (
              <div key={i} style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 12, color: C.blue, marginBottom: 4 }}>You: {x.q}</div>
          ...
          <div style={{ padding: 14, borderTop: `1px solid ${C.border}`, display: "flex", gap: 8 }}>
            ...
            <button className="tiq-btn" disabled={askBusy || !askQ.trim()} onClick={() => ask(m.id, askQ)} style={{ background: askBusy || !askQ.trim() ? C.muted + "30" : m.color, border: "none", borderRadius: 8, color: C.bg, fontFamily: C.display, fontWeight: 800, fontSize: 12, padding: "0 15px", textTransform: "uppercase" }}>Ask</button>
          </div>
        </div>
      ); })()}
    </div>
