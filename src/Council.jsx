...
const SEAT_POS = COUNCIL.map((_, i) => {
  ...
  return { left: 50 + 40.5 * Math.cos(a), top: 45 + 36 * Math.sin(a) };
});
...
export default function Council({ theme: C, db, supabaseReady, userId, holdings, journal, reviews, opportunities, watchlist }) {
  ...
  // ── Seat — a little cartoon character: high-back chair, suited body with the
  // member's color, big emoji head with squash-and-stretch idle, brass nameplate.
  ...
  const Seat = (m, i) => {
    ...
    const v = voteOf[m.id];
    const chair = m.id === "chairman";
    const head = chair ? 38 : 32, bodyW = chair ? 62 : 52;
    return (
      <div key={m.id} className="c-seat" onClick={() => setDossier(m.id)} title={observer ? `${m.name} — observing this session` : `${m.name} — ${m.title}`}
        style={{ position: "absolute", left: `${SEAT_POS[i].left}%`, top: `${SEAT_POS[i].top}%`, transform: `translate(-50%,-50%) scale(${speaking ? 1.18 : 1})`, opacity: observer ? 0.3 : dimmed ? 0.5 : 1, filter: observer ? "grayscale(0.75)" : dimmed ? "saturate(0.5)" : "none", zIndex: speaking ? 6 : 3, textAlign: "center" }}>
        <div style={{ position: "relative", animation: speaking ? "cTalk 0.65s ease-in-out infinite" : observer ? "none" : `cBob ${3.2 + (i % 4) * 0.5}s ease-in-out ${i * 0.37}s infinite`, transformOrigin: "50% 100%" }}>
          {speaking && <div style={{ position: "absolute", left: "50%", top: "38%", transform: "translate(-50%,-50%)", width: bodyW + 52, height: bodyW + 52, borderRadius: "50%", border: `2px dashed ${m.color}55`, animation: "cRing 6s linear infinite", zIndex: 0 }} />}
          {/* chair back */}
          <div style={{ position: "absolute", left: "50%", top: chair ? -9 : -6, transform: "translateX(-50%)", width: bodyW + 24, height: bodyW + 30, borderRadius: "46% 46% 26% 26%", background: `linear-gradient(180deg, ${m.color}30, #0c1522 62%)`, border: "3px solid #02050a", boxShadow: `inset 0 6px 12px ${m.color}26, inset 0 0 0 2px ${m.color}18, 0 9px 16px #000a`, zIndex: 0 }}>
            <div style={{ position: "absolute", left: "50%", top: 5, transform: "translateX(-50%)", width: "55%", height: 7, borderRadius: 5, background: m.color + "33" }} />
          </div>
          {/* head */}
          ...
          <div style={{ margin: "5px auto 0", width: "max-content", padding: "2px 9px", borderRadius: 6, background: "linear-gradient(180deg,#1c2a42,#0d1726)", border: "2px solid #02050a", boxShadow: `inset 0 0 0 1px ${C.gold}38, 0 3px 6px #0009` }}>
            <span style={{ fontFamily: serif, fontWeight: 700, fontSize: 10.5, color: speaking ? m.color : C.text }}>{m.name}</span>
          </div>
          <div style={{ marginTop: 2, fontSize: 7, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: observer ? C.muted : m.color + "aa" }}>{observer ? "observing" : m.title.replace("The ", "")}</div>
          {phase === "debate" && !speaking && reactionOf[m.id] && <div key={turnIdx} style={{ position: "absolute", right: -12, top: -16, fontSize: 15, animation: "cReact 2.8s ease forwards", pointerEvents: "none", zIndex: 8, filter: "drop-shadow(0 2px 4px #000)" }}>{reactionOf[m.id]}</div>}
          {v && <div style={{ marginTop: 3, display: "inline-block", fontSize: 8, fontWeight: 800, letterSpacing: "0.08em", padding: "2px 7px", borderRadius: 8, background: VOTE_COLOR[v.vote] + "26", color: VOTE_COLOR[v.vote], border: `1.5px solid ${VOTE_COLOR[v.vote]}66`, animation: "cVote 0.45s ease", fontFamily: C.display, textShadow: "0 1px 4px #000" }}>{v.vote.toUpperCase()}</div>}
        </div>
      </div>
    );
  };

  // ── Center stage (table center content by phase) ──
  const CenterStage = () => {
    if (activePower) return null;
    if (phase === "verdict" && session) {
      const vd = session.verdict;
      return (
        <div style={{ position: "absolute", left: "50%", top: "45%", transform: "translate(-50%,-50%)", width: "min(480px,82%)", zIndex: 8, animation: "cBubble 0.5s ease" }}>
          <div style={{ background: `linear-gradient(165deg,${C.s2}f8,${C.s1}f8)`, border: `1px solid ${VOTE_COLOR[vd.recommendation]}55`, borderRadius: 14, padding: 18, boxShadow: `0 24px 70px #000c, 0 0 50px ${VOTE_COLOR[vd.recommendation]}18`, maxHeight: "70vh", overflowY: "auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              <span style={{ fontSize: 26, display: "inline-block", animation: "cGavel 1.6s ease 1" }}>🦉</span>
              <div>
                <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.22em", color: C.muted }}>THE CHAIRMAN'S VERDICT</div>
                <div style={{ fontFamily: serif, fontWeight: 900, fontSize: 24, color: VOTE_COLOR[vd.recommendation], lineHeight: 1.1 }}>{vd.recommendation} <span style={{ fontSize: 13, color: C.text, fontWeight: 500 }}>· {vd.confidence}% confidence</span></div>
              </div>
            </div>
            <div style={{ display: "flex", height: 7, borderRadius: 4, overflow: "hidden", marginBottom: 10, border: `1px solid ${C.border}` }}>
              {session.votes.map((v) => <div key={v.member} style={{ flex: 1, background: VOTE_COLOR[v.vote] }} title={`${memberById(v.member).name}: ${v.vote}`} />)}
            </div>
            <div style={{ fontSize: 10, color: C.muted, marginBottom: 6 }}>{tally.consensus} · council score {tally.avg >= 0 ? "+" : ""}{tally.avg}</div>
            {(() => { const co = coalitions(session.votes); return (
              <div style={{ fontSize: 9.5, marginBottom: 10, lineHeight: 1.7 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
              <div><div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.16em", color: "#69f0ae", marginBottom: 3 }}>🐂 BULL CASE</div><div style={{ fontSize: 10.5, lineHeight: 1.55, color: C.text }}>{vd.bull_case}</div></div>
              <div><div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.16em", color: "#ff5252", marginBottom: 3 }}>🐻 BEAR CASE</div><div style={{ fontSize: 10.5, lineHeight: 1.55, color: C.text }}>{vd.bear_case}</div></div>
            </div>
            {vd.key_risks.length > 0 && <div style={{ marginBottom: 8 }}><div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.16em", color: C.gold, marginBottom: 3 }}>KEY RISKS</div>{vd.key_risks.map((x, i) => <div key={i} style={{ fontSize: 10, color: C.text, lineHeight: 1.5 }}>⚠ {x}</div>)}</div>}
            {vd.required_research.length > 0 && <div style={{ marginBottom: 8 }}><div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.16em", color: "#ffd54f", marginBottom: 3 }}>🕵️ REQUIRED RESEARCH</div>{vd.required_research.map((x, i) => <div key={i} style={{ fontSize: 10, color: C.text, lineHeight: 1.5 }}>☐ {x}</div>)}</div>}
            <div style={{ background: C.accent + "10", border: `1px solid ${C.accent}30`, borderRadius: 7, padding: "8px 11px", marginBottom: 11 }}>
              <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.16em", color: C.accent }}>NEXT ACTION → </span>
              <span style={{ fontSize: 10.5, color: C.text }}>{vd.next_action}</span>
            </div>
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <button className="tiq-btn" onClick={() => { setPhase("idle"); setSession(null); setTopic(null); }} style={{ background: C.accent, border: "none", borderRadius: 6, color: C.bg, fontFamily: C.display, fontWeight: 800, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", padding: "8px 14px" }}>⚖ New Session</button>
              {source !== "live" && <button className="tiq-btn" onClick={() => convene(topic, { force: true, mode: session.mode || "quick" })} style={{ background: "none", border: `1px solid ${C.gold}50`, borderRadius: 6, color: C.gold, fontFamily: C.display, fontWeight: 800, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", padding: "7px 12px" }}>↻ Convene Fresh</button>}
              {source === "live" && session.mode === "quick" && <button className="tiq-btn" onClick={() => convene(topic, { force: true, mode: "full" })} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 6, color: C.muted, fontFamily: C.display, fontWeight: 800, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", padding: "7px 12px" }}>🏛️ Escalate to Full Council</button>}
              <span style={{ fontSize: 9, color: C.muted }}>Click any member to question their vote</span>
            </div>
          </div>
        </div>
      );
    ...
    if (speakingTurn) {
      ...
      return (
        <div key={turnIdx} style={{ position: "absolute", left: "50%", top: "45%", transform: "translate(-50%,-50%)", width: "min(440px,80%)", zIndex: 7, animation: "cBubble 0.35s ease" }}>
          <div style={{ background: `linear-gradient(170deg,${C.s2}f5,${C.s1}f5)`, border: `1px solid ${m.color}55`, borderRadius: "16px 16px 16px 4px", padding: "13px 16px", boxShadow: `0 18px 50px #000a, 0 0 36px ${m.color}14` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 6 }}>
              <span style={{ fontSize: 16 }}>{m.emoji}</span>
              <span style={{ fontFamily: serif, fontWeight: 700, fontSize: 13, color: m.color }}>{m.name}</span>
              <span style={{ fontSize: 8, color: C.muted, letterSpacing: "0.14em", textTransform: "uppercase" }}>{m.title}</span>
              {kd && <span style={{ marginLeft: "auto", fontSize: 7.5, fontWeight: 800, letterSpacing: "0.12em", color: m.color + "bb", border: `1px solid ${m.color}40`, borderRadius: 3, padding: "1px 6px" }}>{kd}</span>}
            </div>
            <div style={{ fontSize: 12.5, lineHeight: 1.65, color: C.text, fontFamily: C.mono, minHeight: 20 }}>
    ...
    if (phase === "voting") return (
      <div style={{ position: "absolute", left: "50%", top: "45%", transform: "translate(-50%,-50%)", zIndex: 7, textAlign: "center", animation: "cBubble 0.4s ease" }}>
        <div style={{ fontFamily: serif, fontWeight: 900, fontSize: 22, color: C.gold, textShadow: `0 0 30px ${C.gold}55` }}>THE COUNCIL VOTES</div>
        <div style={{ fontSize: 10, color: C.muted, marginTop: 4 }}>{votesShown} / {session?.votes.length ?? 10} ballots in</div>
      </div>
    );
    return null;
  };
    ...
    <div style={{ display: "flex", height: "100%", minHeight: 480, background: `radial-gradient(120% 90% at 50% 0%, #0c1426 0%, ${C.bg} 55%, #04060a 100%)`, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, position: "relative", minWidth: 0 }}>
        ...
        <div style={{ position: "absolute", top: 12, left: 0, right: 0, textAlign: "center", zIndex: 9, pointerEvents: "none" }}>
          <div style={{ fontFamily: serif, fontWeight: 900, fontSize: 19, letterSpacing: "0.04em", color: C.text }}>🏛️ THE INVESTMENT COUNCIL</div>
          {topic && phase !== "idle" && <div style={{ fontSize: 10, color: C.muted, marginTop: 2, maxWidth: 560, margin: "2px auto 0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{TOPIC_TYPES.find((x) => x.id === topic.type)?.emoji} {topic.title}{session?.mode === "quick" && <span style={{ color: C.accent, marginLeft: 6 }}>⚡ quick panel</span>}{source !== "live" && <span style={{ color: C.gold, marginLeft: 6 }}>↺ cached · 0 tokens</span>}</div>}
          {phase === "idle" && <div style={{ fontSize: 10, color: C.muted, marginTop: 2, fontStyle: "italic" }}>Ten minds. One verdict. No capital committed without a debate.</div>}
        </div>
        {(phase === "debate" || phase === "powers" || phase === "voting") && (
          <button className="tiq-btn" onClick={skip} style={{ position: "absolute", top: 14, right: 14, zIndex: 10, background: C.s2, border: `1px solid ${C.border}`, borderRadius: 5, color: C.muted, fontFamily: C.display, fontWeight: 700, fontSize: 9, letterSpacing: "0.1em", textTransform: "uppercase", padding: "5px 10px" }}>Skip ≫</button>
        )}

        {/* 🏛 Council Treasury — themed usage ledger, health relative to own pattern */}
        <button className="tiq-btn" onClick={() => setShowTreasury((p) => !p)} style={{ position: "absolute", top: 14, left: 14, zIndex: 10, background: C.s2 + "e8", border: `1.5px solid ${healthColor}45`, borderRadius: 6, color: healthColor, fontFamily: C.display, fontWeight: 800, fontSize: 9, letterSpacing: "0.1em", textTransform: "uppercase", padding: "5px 10px", boxShadow: "0 4px 10px #0008" }}>🏛 Treasury <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: healthColor, marginLeft: 3 }} /></button>
        {showTreasury && (
          <div style={{ position: "absolute", top: 46, left: 14, zIndex: 13, width: 252, background: `linear-gradient(185deg,${C.s2}fa,${C.s1}fa)`, border: `2px solid ${C.gold}30`, borderRadius: 10, padding: 13, boxShadow: "0 18px 50px #000b", animation: "cBubble 0.25s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <span style={{ fontFamily: serif, fontWeight: 900, fontSize: 12, color: C.gold }}>🏛 COUNCIL TREASURY</span>
              <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.1em", padding: "2px 7px", borderRadius: 8, background: healthColor + "1c", color: healthColor, border: `1px solid ${healthColor}45` }}>{treasury.h.label.toUpperCase()}</span>
            </div>
            {[["⚡ Quick sessions", treasury.s.quick], ["🏛️ Full sessions", treasury.s.full], ["💬 Member queries", treasury.s.asks],
              ...
              <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 9.5, padding: "3px 0", borderBottom: `1px dashed ${C.border}55` }}>
                <span style={{ color: C.muted }}>{l}</span><span style={{ color: C.text, fontFamily: C.mono, fontWeight: 700 }}>{v}</span>
              </div>
            ))}
            <div style={{ fontSize: 8, color: C.muted, marginTop: 7, lineHeight: 1.5 }}>Last 30 days · real Groq usage when reported · health is measured against your own pattern, not a hardcoded limit{treasury.h.ratio > 0 ? ` (today ≈ ${treasury.h.ratio}× your usual day)` : ""}.</div>
          </div>
        )}

        {/* Sterling's gavel */}
        {gavel && <div style={{ position: "absolute", left: "50%", top: "42%", zIndex: 12, fontSize: 64, animation: "cSlam 1.25s ease forwards", pointerEvents: "none", filter: "drop-shadow(0 10px 18px #000c)" }}>🔨</div>}

        {/* carpet under the table */}
        <div style={{ position: "absolute", left: "50%", top: "49%", transform: "translate(-50%,-50%)", width: "82%", height: "62%", borderRadius: "50%", background: `radial-gradient(ellipse, #122036aa 0%, #0c172855 55%, transparent 74%)`, border: "2px solid #ffffff08", pointerEvents: "none" }} />
        {/* the table — cartoon wood: thick outline, grain rings, glossy swoosh, gold inlay, props */}
        <div style={{ position: "absolute", left: "50%", top: "45%", transform: "translate(-50%,-50%)", width: "62%", height: "46%", borderRadius: "50%", background: "radial-gradient(ellipse at 50% 30%, #7a5530 0%, #5d3e20 36%, #432c14 68%, #2c1c0c 100%)", border: "5px solid #140b04", boxShadow: `0 30px 70px #000c, inset 0 12px 26px #ffffff1a, inset 0 -18px 34px #00000099, 0 0 70px ${C.accent}0a` }}>
          ...
          {["📄", "☕", "📊", "🗂️", "🖋️", "📈", "☕", "📑"].map((p, i) => { const a = (i / 8) * Math.PI * 2 + 0.45; return (
            <span key={i} style={{ position: "absolute", left: `${50 + 35 * Math.cos(a)}%`, top: `${50 + 35 * Math.sin(a)}%`, transform: "translate(-50%,-50%)", fontSize: 12, filter: "drop-shadow(0 3px 3px #000a)", pointerEvents: "none" }}>
              {p}{p === "☕" && <span style={{ position: "absolute", left: 3, top: -7, fontSize: 7, animation: `cSteam 2.2s ease-out ${i * 0.7}s infinite`, display: "inline-block" }}>〰</span>}
            </span>); })}
          {phase === "idle" && !busy && (
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
              <div style={{ fontSize: 30, opacity: 0.85 }}>⚖️</div>
              <div style={{ fontFamily: serif, fontWeight: 700, fontSize: 13, color: C.gold + "cc", marginTop: 4, letterSpacing: "0.12em" }}>SESSION NOT IN PROGRESS</div>
              <div style={{ fontSize: 9.5, color: C.muted, marginTop: 3, maxWidth: 260 }}>Bring an opportunity, trade, position, or thesis before the council below.</div>
              {cMode === "quick" && <div style={{ fontSize: 8.5, color: C.accent + "bb", marginTop: 5 }}>⚡ Quick panel: Sterling · Veris · Sigma · Popper</div>}
            </div>
          )}
          {busy && (
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div className="spin" style={{ width: 26, height: 26, border: `3px solid ${C.gold}25`, borderTop: `3px solid ${C.gold}`, borderRadius: "50%" }} />
              <div style={{ fontFamily: serif, fontWeight: 700, fontSize: 12, color: C.gold, marginTop: 10, letterSpacing: "0.1em" }}>THE COUNCIL IS CONVENING…</div>
              <div style={{ fontSize: 9, color: C.muted, marginTop: 3 }}>reviewing your portfolio, track record & theses</div>
            </div>
          )}
        </div>
        ...
        {activePower && (
          <div style={{ position: "absolute", inset: 0, zIndex: 11, display: "flex", alignItems: "center", justifyContent: "center", background: "#04060acc", backdropFilter: "blur(2px)" }}>
            <div style={{ width: "min(440px,86%)", textAlign: "center", background: `linear-gradient(170deg,${C.s2},${C.s1})`, border: `2px solid ${activePower.color}66`, borderRadius: 16, padding: "26px 24px", boxShadow: `0 0 80px ${activePower.color}30`, animation: "cPower 0.5s ease" }}>
              <div style={{ fontSize: 42, animation: "cGavel 1.4s ease infinite" }}>{activePower.emoji}</div>
              <div style={{ fontFamily: serif, fontWeight: 900, fontSize: 23, color: activePower.color, letterSpacing: "0.1em", margin: "6px 0 4px" }}>{activePower.title}</div>
              <div style={{ fontSize: 9, color: C.muted, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 10 }}>{activePower.sub}</div>
              <div style={{ fontSize: 12, lineHeight: 1.7, color: C.text, fontFamily: C.mono }}>{activePower.text}</div>
            </div>
          </div>
        )}
        ...
        {phase === "idle" && (
          <div style={{ position: "absolute", left: "50%", bottom: 14, transform: "translateX(-50%)", width: "min(620px,94%)", zIndex: 9 }}>
            {err && <div style={{ background: C.red + "15", border: `1px solid ${C.red}40`, borderRadius: 6, padding: "7px 11px", fontSize: 10.5, color: C.red, marginBottom: 8 }}>⚠️ {err}</div>}
            {suggestions.length > 0 && (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8, justifyContent: "center" }}>
                {suggestions.map((s, i) => <button key={i} className="tiq-btn" onClick={() => convene(s.t)} style={{ background: C.s2 + "ee", border: `1px solid ${C.accent}30`, borderRadius: 14, color: C.accent, fontFamily: C.mono, fontSize: 9.5, padding: "5px 11px" }}>{s.e} {s.l}</button>)}
                {lastSaved && <button className="tiq-btn" onClick={() => replay(lastSaved)} style={{ background: C.s2 + "ee", border: `1px solid ${C.border}`, borderRadius: 14, color: C.muted, fontFamily: C.mono, fontSize: 9.5, padding: "5px 11px" }}>↺ Replay last session</button>}
              </div>
            )}
            <div style={{ background: C.s1 + "f2", border: `1px solid ${C.border}`, borderRadius: 11, padding: 12, boxShadow: "0 14px 44px #000a" }}>
              <div style={{ display: "flex", gap: 7, marginBottom: 9 }}>
                {COUNCIL_MODES.map((md) => <button key={md.id} className="tiq-btn" onClick={() => setCMode(md.id)} style={{ flex: 1, textAlign: "left", background: cMode === md.id ? (md.id === "quick" ? C.accent : C.gold) + "16" : "transparent", border: `1.5px solid ${cMode === md.id ? (md.id === "quick" ? C.accent : C.gold) + "60" : C.border}`, borderRadius: 8, padding: "7px 11px" }}>
                  <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 10.5, letterSpacing: "0.06em", textTransform: "uppercase", color: cMode === md.id ? (md.id === "quick" ? C.accent : C.gold) : C.muted }}>{md.emoji} {md.label}{md.id === "quick" && <span style={{ fontSize: 8, marginLeft: 5, opacity: 0.8 }}>DEFAULT</span>}</div>
                  <div style={{ fontSize: 8.5, color: C.muted, marginTop: 1 }}>{md.hint}</div>
                </button>)}
              </div>
              <div style={{ display: "flex", gap: 5, marginBottom: 9, flexWrap: "wrap" }}>
                {TOPIC_TYPES.map((t) => <button key={t.id} className="tiq-btn" onClick={() => setTType(t.id)} title={t.hint} style={{ background: tType === t.id ? C.accent + "1f" : "transparent", border: `1px solid ${tType === t.id ? C.accent + "60" : C.border}`, borderRadius: 6, color: tType === t.id ? C.accent : C.muted, fontFamily: C.display, fontWeight: 700, fontSize: 9, letterSpacing: "0.06em", textTransform: "uppercase", padding: "5px 9px" }}>{t.emoji} {t.label}</button>)}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <input className="tiq-input" value={tTicker} onChange={(e) => setTTicker(e.target.value.toUpperCase())} placeholder="TICKER" style={{ width: 92, background: C.s2, border: `1px solid ${C.border}`, borderRadius: 6, padding: "9px 10px", color: C.text, fontFamily: C.mono, fontSize: 11 }} />
                <input className="tiq-input" value={tText} onChange={(e) => setTText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && (tText.trim() || tType === "portfolio")) convene({ type: tType, ticker: tTicker.trim(), title: tText.trim() || "Review my full portfolio: exposures, concentration, correlated bets, hidden risks, and what to change." }); }} placeholder={tType === "portfolio" ? "Optional — leave blank for a full portfolio review" : `Describe the ${TOPIC_TYPES.find((x) => x.id === tType).label.toLowerCase()} to debate…`} style={{ flex: 1, background: C.s2, border: `1px solid ${C.border}`, borderRadius: 6, padding: "9px 12px", color: C.text, fontFamily: C.mono, fontSize: 11.5 }} />
                <button className="tiq-btn" disabled={!tText.trim() && tType !== "portfolio"} onClick={() => convene({ type: tType, ticker: tTicker.trim(), title: tText.trim() || "Review my full portfolio: exposures, concentration, correlated bets, hidden risks, and what to change." })} style={{ background: (tText.trim() || tType === "portfolio") ? `linear-gradient(120deg,${C.gold},${C.accent})` : C.muted + "30", border: "none", borderRadius: 6, color: C.bg, fontFamily: C.display, fontWeight: 800, fontSize: 10.5, letterSpacing: "0.08em", textTransform: "uppercase", padding: "0 16px", cursor: (tText.trim() || tType === "portfolio") ? "pointer" : "not-allowed" }}>⚖ Convene</button>
              </div>
            </div>
          </div>
        )}
      ...
      {/* ── MINUTES RAIL ── */}
      <div style={{ width: 330, flexShrink: 0, borderLeft: `1px solid ${C.border}`, background: C.s1 + "c8", display: "flex", flexDirection: "column", backdropFilter: "blur(4px)" }}>
        <div style={{ padding: "11px 14px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontFamily: serif, fontWeight: 700, fontSize: 11, letterSpacing: "0.14em", color: C.muted }}>MINUTES OF THE MEETING</span>
          {session && <span style={{ fontSize: 8.5, color: C.muted }}>{playedTurns.length}/{session.transcript.length}</span>}
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px" }}>
          {!session && <div style={{ color: C.muted, fontSize: 10.5, lineHeight: 1.7, padding: "16px 4px", textAlign: "center" }}>The live transcript, ballots, and verdict will be recorded here once the council convenes.<div style={{ marginTop: 14, fontSize: 9, lineHeight: 2 }}>{COUNCIL.map((m) => <div key={m.id} style={{ cursor: "pointer" }} onClick={() => setDossier(m.id)}><span style={{ color: m.color }}>{m.emoji} {m.name}</span> · {m.title}</div>)}</div></div>}
          {playedTurns.map((t, i) => (
            <div key={i} className="msg-in" style={{ marginBottom: 9, paddingLeft: 8, borderLeft: `2px solid ${memberById(t.member).color}50` }}>
              <div style={{ fontSize: 9 }}><MChip id={t.member} />{KIND_TAG[kindOf(t, i, session.transcript)] && <span style={{ color: C.muted, marginLeft: 5, fontSize: 7.5, letterSpacing: "0.1em" }}>{KIND_TAG[kindOf(t, i, session.transcript)]}</span>}</div>
              <div style={{ fontSize: 10.5, color: C.text, lineHeight: 1.55, marginTop: 2 }}>{t.text}</div>
            </div>
          ))}
          {session?.evidence_hold?.raised && phase !== "debate" && <div className="msg-in" style={{ margin: "10px 0", padding: "7px 10px", background: "#ffd54f12", border: "1px solid #ffd54f3a", borderRadius: 6, fontSize: 9.5, color: "#ffd54f" }}>🕵️ EVIDENCE MISSING — {session.evidence_hold.demand}</div>}
          {session?.red_flags?.raised && phase !== "debate" && <div className="msg-in" style={{ margin: "10px 0", padding: "7px 10px", background: "#f48fb112", border: "1px solid #f48fb13a", borderRadius: 6, fontSize: 9.5, color: "#f48fb1" }}>⚖️ RED FLAG REVIEW — {(session.red_flags.flags || []).join(" · ")}</div>}
          {shownVotes.length > 0 && <div style={{ margin: "12px 0 6px", fontSize: 8.5, fontWeight: 800, letterSpacing: "0.18em", color: C.muted }}>— BALLOTS —</div>}
          {shownVotes.map((v) => (
            <div key={v.member} className="msg-in" style={{ display: "flex", gap: 7, alignItems: "baseline", marginBottom: 6 }}>
              <span style={{ fontSize: 9, minWidth: 74 }}><MChip id={v.member} /></span>
              <span style={{ fontSize: 9, fontWeight: 800, color: VOTE_COLOR[v.vote], whiteSpace: "nowrap" }}>{v.vote} · {v.confidence}%</span>
              <span style={{ fontSize: 9, color: C.muted, lineHeight: 1.45 }}>{v.reason}</span>
            </div>
          ))}
          {phase === "verdict" && session && <div className="msg-in" style={{ marginTop: 10, padding: "8px 11px", background: VOTE_COLOR[session.verdict.recommendation] + "12", border: `1px solid ${VOTE_COLOR[session.verdict.recommendation]}3a`, borderRadius: 6, fontSize: 10, color: C.text }}>🦉 <b style={{ color: VOTE_COLOR[session.verdict.recommendation] }}>{session.verdict.recommendation}</b> ({session.verdict.confidence}%) — {session.verdict.next_action}</div>}
          <div ref={feedEnd} />
        </div>
      </div>
      ...
      {dossier && (() => { const m = memberById(dossier); const v = session?.votes.find((x) => x.member === m.id); const th = threads[m.id] || []; return (
        <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "min(360px,94%)", zIndex: 20, background: `linear-gradient(190deg,${C.s2}fb,${C.s1}fb)`, borderLeft: `2px solid ${m.color}50`, boxShadow: "-22px 0 60px #000b", display: "flex", flexDirection: "column", animation: "cBubble 0.3s ease" }}>
          <div style={{ padding: "16px 16px 12px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 25, background: `radial-gradient(circle at 32% 28%, ${m.color}3a, ${C.s2} 66%)`, border: `2px solid ${m.color}60`, animation: "cBob 3.4s ease-in-out infinite" }}>{m.emoji}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: serif, fontWeight: 900, fontSize: 17, color: m.color }}>{m.name}</div>
                <div style={{ fontSize: 8.5, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: C.muted }}>{m.title}</div>
              </div>
              <button className="tiq-btn" onClick={() => setDossier(null)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 5, color: C.muted, fontSize: 11, padding: "3px 8px" }}>✕</button>
            </div>
            {v && <div style={{ marginTop: 9, padding: "6px 10px", borderRadius: 6, background: VOTE_COLOR[v.vote] + "14", border: `1px solid ${VOTE_COLOR[v.vote]}40`, fontSize: 10 }}><b style={{ color: VOTE_COLOR[v.vote] }}>{v.vote}</b> <span style={{ color: C.muted }}>· {v.confidence}% —</span> <span style={{ color: C.text }}>{v.reason}</span></div>}
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px" }}>
            <div style={{ fontSize: 10.5, color: C.text, lineHeight: 1.65, fontStyle: "italic", marginBottom: 8 }}>"{m.philosophy}"</div>
            <div style={{ fontSize: 10, color: C.muted, lineHeight: 1.6, marginBottom: 8 }}>{m.focus}</div>
            {m.power && <div style={{ fontSize: 9.5, color: m.color, background: m.color + "10", border: `1px solid ${m.color}30`, borderRadius: 6, padding: "6px 9px", marginBottom: 10 }}>⚡ {m.power}</div>}
            {memories[m.id] && <div style={{ background: C.s2, border: `1px dashed ${m.color}38`, borderRadius: 6, padding: "7px 10px", marginBottom: 10 }}>
              <div style={{ fontSize: 7.5, fontWeight: 800, letterSpacing: "0.16em", color: m.color, marginBottom: 3 }}>{m.name.toUpperCase()}'S READ ON YOU</div>
              <div style={{ fontSize: 9.5, color: C.text, lineHeight: 1.6 }}>{memories[m.id]}</div>
            </div>}
            <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 12 }}>
              {m.questions.map((q) => <button key={q} className="tiq-btn" disabled={askBusy} onClick={() => ask(m.id, q)} style={{ textAlign: "left", background: C.s3, border: `1px solid ${C.border}`, borderRadius: 6, color: C.accent, fontFamily: C.mono, fontSize: 9.5, padding: "6px 10px" }}>❝ {q} ❞</button>)}
            </div>
            {th.map((x, i) => (
              <div key={i} style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 10, color: C.blue, marginBottom: 3 }}>You: {x.q}</div>
                <div style={{ fontSize: 10.5, color: C.text, lineHeight: 1.6, background: C.s3, border: `1px solid ${m.color}28`, borderRadius: "10px 10px 10px 3px", padding: "8px 11px" }}>{x.a == null ? <span style={{ color: C.muted }}>{m.emoji} thinking…</span> : x.a}</div>
              </div>
            ))}
          </div>
          <div style={{ padding: 12, borderTop: `1px solid ${C.border}`, display: "flex", gap: 7 }}>
            <input className="tiq-input" value={askQ} onChange={(e) => setAskQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && ask(m.id, askQ)} placeholder={`Ask ${m.name} anything…`} style={{ flex: 1, background: C.s2, border: `1px solid ${C.border}`, borderRadius: 6, padding: "8px 11px", color: C.text, fontFamily: C.mono, fontSize: 11 }} />
            <button className="tiq-btn" disabled={askBusy || !askQ.trim()} onClick={() => ask(m.id, askQ)} style={{ background: askBusy || !askQ.trim() ? C.muted + "30" : m.color, border: "none", borderRadius: 6, color: C.bg, fontFamily: C.display, fontWeight: 800, fontSize: 10, padding: "0 13px", textTransform: "uppercase" }}>Ask</button>
          </div>
        </div>
      ); })()}
    </div>
