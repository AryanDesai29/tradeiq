...
export default function TradeReview({ review, trade, theme, onRegenerate, onVerdict }) {
  ...
  if (review.error) return <div style={{ fontSize: 12, color: C.red, marginTop: 8 }}>⚠️ Review failed: {review.error}</div>;
  ...
  const L = ({ title, items, color }) => (items && items.length ? (
    <div style={{ flex: 1, minWidth: 150 }}>
      <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color, marginBottom: 4 }}>{title}</div>
  ...
  return (
    <div style={{ marginTop: 10, background: C.s2, border: `1px solid ${C.border}`, borderRadius: 6, padding: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
        ...
        <div>
          <div style={{ fontFamily: C.display, fontWeight: 700, fontSize: 12, color: C.text }}>AI Trade Review</div>
          <div style={{ fontSize: 12, fontWeight: 700, color: verdictColor }}>{VERDICT_LABEL[verdict] || verdict}</div>
        </div>
        <div style={{ marginLeft: "auto", fontSize: 12, color: C.muted }}><Term k="process">Process</Term> {proc}/100</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(90px,1fr))", gap: 8, marginBottom: 10 }}>
        {SCORES.map(([l, v]) => (
          <div key={l}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.muted, marginBottom: 2 }}><span>{l}</span><span style={{ color: bar(v), fontWeight: 700 }}>{v}</span></div>
        ...
        return (
          <div style={{ marginBottom: 10, background: C.s1, border: `1px solid ${col}40`, borderLeft: `3px solid ${col}`, borderRadius: 6, padding: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, flexWrap: "wrap", gap: 6 }}>
              <span style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: C.muted }}>Thesis Verdict</span>
            ...
            {trade && (trade.expectations || trade.reality) && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 8, marginBottom: 8 }}>
                <div><div style={{ fontSize: 12, color: C.muted, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 2 }}>Market expected</div><div style={{ fontSize: 13, color: C.text, lineHeight: 1.45 }}>{trade.expectations || "—"}</div></div>
                <div><div style={{ fontSize: 12, color: C.green, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 2 }}>What happened</div><div style={{ fontSize: 13, color: C.text, lineHeight: 1.45 }}>{review.thesis_reason || trade.reality || "—"}</div></div>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 10, fontSize: 12, marginBottom: onVerdict ? 8 : 0, flexWrap: "wrap" }}>
            ...
            {onVerdict && (
              <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                <span style={{ fontSize: 12, color: C.muted, marginRight: 2 }}>Your call:</span>
                {THESIS_VERDICTS.map((v) => { const active = fin === v; const vc = tColor[v]; return (
                  <button key={v} onClick={() => onVerdict(v)} style={{ fontSize: 12, fontWeight: 700, padding: "3px 9px", borderRadius: 4, cursor: "pointer", border: `1px solid ${active ? vc : C.border}`, background: active ? vc + "22" : "transparent", color: active ? vc : C.muted }}>
      ...
      {review.tags && review.tags.length > 0 && (
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", borderTop: `1px solid ${C.border}`, paddingTop: 8 }}>
          {review.tags.map((t) => (
            <span key={t} style={{ fontSize: 12, fontWeight: 700, padding: "2px 7px", borderRadius: 3, background: C.red + "15", color: C.red, border: `1px solid ${C.red}30` }}>{MISTAKE_TAGS[t] || t}</span>
          ))}
        </div>
      )}
    </div>
